# Lockyt - a YouTrack App Protecting Issues and Articles 

<img src="./public/icon.svg" alt="Lockyt icon" width="128" height="128">

A YouTrack app that locks resolved issues and allows users to freeze articles.
A locked issue or a frozen article is read-only.
...
The one permitted change is to reopen the issue.
...
| --- | --- | --- |
| Lock a resolved issue | On | Turns the lock on or off for issues in the project. |
...
A user **freezes** an article to lock it. A frozen article is read-only. A user
**unfreezes** the article to edit it again. An article that is not frozen is *editable**.
...
| Setting | Default | Description |
| --- | --- | --- |
| Make a frozen article read-only | On | Turns the lock on or off for articles in the project. |
